...
<table class="table table-bordered table-hover definewidth m10">
    ...
    <tr>
        ...
        <td>
        	...
        	<input type="radio" name="sex" value="0" <c:if test="${doctor.sex == 0 }">checked='checked'</c:if>/>男&nbsp;&nbsp;&nbsp;
        	<input type="radio" name="sex"  value="1" <c:if test="${doctor.sex == 1 }">checked='checked'</c:if>/>女
        </td>
    </tr>
    <tr>
        <td width="10%" class="tableleft">出生年月</td>
        <td><input type="text" name="birthday" class="Wdate" 
        onclick="WdatePicker({el:this,dateFmt:'yyyy-MM-dd',maxDate:'%y-%M-%d'})" value="<fmt:formatDate value="${doctor.birthday }" pattern="yyyy-MM-dd"/>"/></td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>
        	<select name="education">
        		<option value="1" <c:if test="${doctor.education == 1 }">selected='selected'</c:if>>专科</option>
        		<option value="2" <c:if test="${doctor.education == 2 }">selected='selected'</c:if>>本科</option>
        		<option value="3" <c:if test="${doctor.education == 3 }">selected='selected'</c:if>>研究生</option>
        		<option value="4" <c:if test="${doctor.education == 4 }">selected='selected'</c:if>>博士</option>
        	</select>
        </td>
    </tr>
	<tr>
